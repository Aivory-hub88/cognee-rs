...
impl FailureReport {
    ...
    ///   That backstop matters because a file that fails *at the chunk stage*
    ///   produced no chunks, so it contributes to neither side of the ratio —
    ///   a run where every file failed to open would otherwise score 0 %.
    pub fn is_fatal(&self, policy: &FailurePolicy) -> bool {
        ...
        match policy.scope {
            ...
            RollbackScope::FailedItems => {
                if self.total_items > 0 && self.failed_items.len() >= self.total_items {
                    return true;
                }
